...
export function ExampleGroup({ items, layout = 'chips', eyebrow }: Props) {
...
function GridLayout({ items }: { items: ExampleItem[] }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item, i) => (
        ...
        >
          ...
          {(item.translit || item.meaning) && (
            <div className="mt-1.5 space-y-0.5">
              {item.translit && <div className="translit text-xs">{item.translit}</div>}
              {item.meaning && (
                <div className="text-xs text-[var(--color-on-surface-muted)]">{item.meaning}</div>
              )}
            </div>
          )}
          {item.note && (
            <div className="mt-2 text-[11px] italic text-[var(--color-on-surface-faint)]">{item.note}</div>
          )}
        </motion.div>
      ))}
    </div>
